...
let createEvents = function (db, volcanoes) {
  ...
  const samples = []
  const authorsDOI = {}
  fs.createReadStream('./data/TephraDataBase_renormalizado.csv')
    .pipe(csv())
    .on('data', (row) => {
      // COMPUTE EVENTS
      let v = row.Volcano
