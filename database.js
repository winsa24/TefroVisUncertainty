      ...
      db.run(`CREATE TABLE volcano (
        ...
        (err) => {
          if (err) {
            ...
            var insert = 'INSERT INTO volcano (Name, Latitude, Longitude, Color, effusive_regression_a, effusive_regression_b, effusive_regression_SampleNumber, bulk_pyroclastic_regression_a, bulk_pyroclastic_regression_b, bulk_pyroclastic_regression_SampleNumber) VALUES (?,?,?,?,?,?,?,?,?,?)'
            var id = 1;
            for (let v in volcanoes) {
              volcan = volcanoes[v];
              db.run(insert, [volcan.Volcano, volcan.Latitude, volcan.Longitude, volcan.Color, volcan.effusive_regression_a, volcan.effusive_regression_b, volcan.effusive_regression_SampleNumber, volcan.bulk_pyroclastic_regression_a, volcan.bulk_pyroclastic_regression_b, volcan.bulk_pyroclastic_regression_SampleNumber])
              volcan.id = id
              id += 1
            }
...
let createEvents = function (db, volcanoes) {
  ...
  const samples = []
  const authorsDOI = {}
  fs.createReadStream('./data/TephraDataBase_renormalizado_distance_to_fit.csv')
    .pipe(csv())
    .on('data', (row) => {
      // COMPUTE EVENTS
      let v = row.Volcano
